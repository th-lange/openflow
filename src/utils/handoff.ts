...
export function extractHandoff(text: string): string | null {
  ...
  while ((m = pattern.exec(text)) !== null) last = m[1].trim();
 ...
 */
export function compactForThread(text: string): string {
  const handoff = extractHandoff(text);
  if (handoff !== null) return handoff;
  if (text.length <= HANDOFF_FALLBACK_LIMIT) return text;
  const dropped = text.length - HANDOFF_FALLBACK_LIMIT;
  return (
    text.slice(0, HANDOFF_FALLBACK_LIMIT) +
