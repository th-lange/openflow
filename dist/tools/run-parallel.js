...
export async function runParallel(workflow, prompt, context, sessionId, serverUrl) {
    ...
    const { result: mergerOutput } = await delegateTask({ agent: merger, prompt: mergerPrompt, context: resultsContext, sessionId }, serverUrl);
    return [
