...
export async function runEvaluatorOptimizer(workflow, prompt, initialContext, sessionId, client, settings, ledger, signal) {
    ...
    for (let i = 1; i <= maxIterations; i++) {
        ...
        const producerContext = [
            ...
            .filter(Boolean)
            .join("\n\n");
        const { result: producerOutput } = await delegateTask({ agent: producer, prompt, context: producerContext || undefined, sessionId }, client, signal, settings.agentTimeoutMs, ledger);
