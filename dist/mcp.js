...
// ── create_agent ──────────────────────────────────────────────────────────────
server.tool("create_agent", "Create or update an agent definition in opencode.json. OpenCode must reload before the new agent is usable.", {
    ...
    const result = await createAgent({ name, prompt, description, mode, model, allowEdit, allowBash, force }, WORK_DIR);
    return { content: [{ type: "text", text: result }] };
});
// ── enable_workflow ───────────────────────────────────────────────────────────
