...
// ── delegate_task ─────────────────────────────────────────────────────────────
server.tool("delegate_task", "Delegate work to a named OpenCode agent in a child session. Returns the agent's complete text response.", {
    ...
    const output = await delegateTask({ agent, prompt, context, sessionId }, client);
    return { content: [{ type: "text", text: output.result }] };
});
// ── run_workflow ──────────────────────────────────────────────────────────────
server.tool("run_workflow", "Execute a sequential workflow in code. Runs all steps in order, threads context between them, and returns the complete result. Do NOT use for orchestrator workflows — handle those yourself via delegate_task loops as instructed.", {
    ...
    const result = await runWorkflow(name, prompt, context, sessionId, client, WORK_DIR);
    return { content: [{ type: "text", text: result }] };
});
// ── get_workflow ──────────────────────────────────────────────────────────────
server.tool("get_workflow", "Look up a workflow definition by name. Returns the full config including pattern, sequence or agents, and constraints.", {
    name: z.string().describe("Workflow name as defined in openflow.json"),
}, async ({ name }) => {
...
// ── list_workflows ────────────────────────────────────────────────────────────
server.tool("list_workflows", "List workflows defined in openflow.json. By default only enabled workflows are shown.", {
...
// ── create_workflow ───────────────────────────────────────────────────────────
server.tool("create_workflow", "Create or update a workflow definition in openflow.json. Supports all patterns (sequential, orchestrator, evaluator-optimizer, conditional, fanout, parallel, debate). Provide the fields for the chosen pattern; the entry is validated (shape, referenced agents, workflow references, and cycles) before it is persisted.", createWorkflowArgs, async (args) => {
    const result = await createWorkflow(args, client, WORK_DIR);
    return { content: [{ type: "text", text: result }] };
});
// ── create_agent ──────────────────────────────────────────────────────────────
server.tool("create_agent", "Create or update an agent definition in opencode.json. OpenCode must reload before the new agent is usable.", createAgentArgs, async (args) => {
    const result = await createAgent(args, WORK_DIR);
    return { content: [{ type: "text", text: result }] };
});
// ── enable_workflow ───────────────────────────────────────────────────────────
server.tool("enable_workflow", "Enable a previously disabled workflow so it appears in list_workflows and can be run.", {
    name: z.string().describe("Workflow name as defined in openflow.json"),
}, async ({ name }) => {
...
// ── disable_workflow ──────────────────────────────────────────────────────────
server.tool("disable_workflow", "Disable a workflow so it is hidden from list_workflows and cannot be run. The definition is preserved and can be re-enabled later.", {
    name: z.string().describe("Workflow name as defined in openflow.json"),
}, async ({ name }) => {
